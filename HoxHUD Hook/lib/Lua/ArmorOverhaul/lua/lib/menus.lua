...
m[#m+1] = menu_checkbox{
    text_id = "armor_overhaul_options_localize",
    help_id = "armor_overhaul_options_localize_help",
    key = 'example',
    default = ArmorOverhaul and ArmorOverhaul.options.gui_enabled or 'on',
    callback = function(self, item)
        LAOUtils.change_option("localize", item:value())
        LAOUtils.save_options()
    end
}
m[#m+1] = menu_item{
...
for tab_name, tab in pairs(crews) do
	for k, v in pairs(tab) do
		table.insert(m[#m], {
			_meta = "option",
			text_id = (ArmorOverhaul.options.localize and "crew_bonus_" or "") .. tab_name .. "_" .. k,
			value = tab_name .. "_" .. k
		})
	end
end
